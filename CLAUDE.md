...
- **Inference endpoints are per-agent in user/project config**: `agents.<name>.endpoints: [{protocol, url, add_host, provider, prefer, ca_cert, no_auth}]`. `protocol` is `openai` or `anthropic`; sandy translates each entry into env vars (`OPENAI_BASE_URL`/`ANTHROPIC_BASE_URL`) and adds `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` to env passthrough. `add_host` parses the URL hostname and injects `--add-host`. Per agent, at most one endpoint per protocol after merge; project entries replace user entries by `(agent, protocol)`. ANTHROPIC_BASE_URL is omitted when the URL is the default `https://api.anthropic.com` to keep claude on the standard cloud client path. No CLI override - edit YAML.
- **`no_auth` endpoints get a placeholder key, never the host's**: a self-hosted server usually needs no credential, but agents disagree - Qwen Code opens its provider-login wizard and refuses to start. `no_auth: true` makes `sandbox.applyAPIKey` set `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` to the literal `sandy-no-auth` in `RunSpec.Env` and skip the passthrough entirely, so the host's real cloud key is never handed to someone else's box (the host-side `/models` probe drops the header too). This is the one case where a key-named variable carries a value in `Env`; it is a public placeholder, not a secret, so the passthrough invariant still holds. `Build` also skips any manifest `env_passthrough` name that already has an explicit `Env` value, so docker never sees two conflicting `-e` flags for one name.
- **`ca_cert` trusts an internal CA on both sides**: an endpoint may carry `ca_cert`, a host path to a PEM bundle. Host-side discovery adds it to the system roots (an explicit `RootCAs` pool also switches Go off the platform verifier, which is what lets a long-lived internal leaf work on macOS - Security.framework rejects >398-day TLS certs as "not standards compliant" no matter how the CA is trusted). Container-side it is bind-mounted read-only at the constant `/etc/sandy/ca/endpoint.crt` with `NODE_EXTRA_CA_CERTS` pointing at it - additive, unlike `SSL_CERT_FILE`, which would replace the whole bundle. The container path is never derived from the configured path, and `config.ResolveCACert` requires the file to hold a PEM certificate, so a project config cannot use it to pull an arbitrary host file into the container. One bundle per agent (Node reads one file); a second, different one errors.
