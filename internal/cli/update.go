...
func newUpdateCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "update [agent]",
		Short: "Pull the latest image(s) for an agent (or all agents if omitted)",
		Long: `Pull the latest sandbox images. This fetches the base image,
toolchain image, and all agent images (or just the specified agent's image
